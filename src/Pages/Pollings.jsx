...
const Pollings = (props) => {
  ...
  const toPolling = (e) => {
    ...
    console.log({val: id});
    try {
      let response = fetch(`${API_KEY}/poll/${id}`, {headers: headersApi}).then(res => res.json()).then((res) => {
        console.log({res});
        return res;
      })

      console.log({resVote: response});
    } catch (e) {
      console.log({err: e});
    }
  } 
      ...
      {loading && <Loading/>}
        <div className="container mt-2" style={{overflowX: 'hidden'}}>
          <div className="row border border-1 rounded d-flex justify-content-center p-2 gap-4">
            {pollings.map((poll, i) => {
              return (
                <div key={i} className="col-md-6 border border-1 rounded p-2" style={{maxWidth: '25rem'}}>
                  ...
                  <div className="d-flex justify-content-end p-1">
                    <button className="btn btn-primary btn-sm" value={poll.id} onClick={toPolling} style={{width: "120px"}}>Vote</button>
                  </div>
                </div>
              )
            })}
